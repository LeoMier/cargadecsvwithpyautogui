...
path_i = r'C:\Users\JF_db\Desktop\database\paymentbills\Data\upload'
path_f = r'C:\Users\JF_db\Desktop\database\paymentbills\Data\done'
...
for _ in range(len(os.listdir(path_i))):
    ...
    while u_completed == False:
        ...
        time.sleep(6)
        pyautogui.typewrite('PaymentBill')
        time.sleep(0.5)
        pyautogui.click(339, 283) #click at TimeCard
        validation = wait_for_confirmation('PaymentBill', 273, 246, 371, 264, time_out=10)
        if validation == False:
            reload(True)
            continue
        time.sleep(1)
        ...
        pyautogui.click(671, 496) #chage the shop field
        pyautogui.typewrite('migratio')
        time.sleep(1.5)
        pyautogui.click(668, 528)
        time.sleep(1)
